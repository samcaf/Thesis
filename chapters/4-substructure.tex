\chapter[Removing the Rubble:\\\phantom{Chapter 4:|}Jet Grooming and Cleanup Fish]{Removing the Rubble\\Jet Grooming and Cleanup Fish}
...
%
\sam{funny, but you can say something more reasonable here}
%
...
\remark{jet-discontinuity}{
    All observables depending on a jet definition are technically angularly discontinuous because the angles between particles determine, discontinuously, whether they are reconstructed as within the same jet by the jet algorithm.
    %
    The \gls{kt} and \gls{akt} jet algorithms are soft discontinuous for the same reason.
    %
...
In this chapter, we show that grooming can provide a useful and intuitive way to extract high-energy information from jet substructure.
%
We exposit the physics of traditional, hard-cutoff grooming in \Sec{traditional-grooming} before exploring strategies for continuous grooming -- the \PIRANHA{} paradigm -- in \Sec{piranha}.
%
We emphasize the improved continuity of \PIRANHA{} formally in \Sec{discontinuity} and in practical, phenomenological examples in \Sec{grooming-pheno}.



% TODO: put in table, maybe earlier
...
\footnote{
    ...
    % https://arxiv.org/pdf/1901.10342#figure.caption.47
    % https://arxiv.org/pdf/1901.10342#subfigure.6.4.1
    % Resummation not as simple..?
}
...
\remark{}{
    ...
    %
    We say that the constant cutoff algorithm exhibits a \gls{soft-discontinuity} (defined more thoroughly in \Sec{discontinuity}) because the addition of very small amounts of energy can discontinuously change the result of the grooming.
}
...
\remark{cc-discontinuity}{
    The discontinuity of the \gls{constant-cutoff} grooming algorithm manifests in LL calculations as a \textit{discontinuity in the radiators} of substructure observables, e.g. as in \Eq{ll-cc-mass}.
    %
...
\begin{exercise}
    Argue that, though \(\thetacrit\) is not an additive observable, we may nonetheless use \glspl{substructure-diagram} and \Eq{ll-groomed-additive} to calculate the cumulative distribution for \(\thetacrit\) at LL.
    %
...
In this work, we use Soft Drop in \vocab{grooming mode}:
%
...
\remark{}{
    The first emission of the angular-ordered jet branching history to pass the grooming procedure will be referred to as the \gls{crit-emission}.
    %
    The ungroomed emissions that occur afterward will be called \glspl{sub-emission}.
    %
    When relevant, we will refer to any branches of the jet history \textit{before} the critical emission as \glspl{precrit-emission}.
}


\gls{soft-drop} \gls{declustering} \sam{Check} is a generalization of the modified Mass Drop Tagger (mMDT) algorithm \cite{Dasgupta:2013ihk}, and reproduces mMDT when \(\beta_{\rm SD} = 0\).
%
...
\begin{example}
    ...
    Using the singular pieces of the splitting functions and keeping only leading logarithmic terms in \(\log(m^2/Q^2)\) yields the LL result, for a jet initiated by a parton of color representation \(R\),%
    \footnote{
        The result including non-singular pieces of splitting functions can be found e.g. in \href{https://arxiv.org/pdf/1402.2657\#equation.3.9}{Eq. (3.9)} of \Reff{Larkoski:2014wba} by setting their \(\beta = \betasd\), \(\alpha = 2\).
    }
    %
    \begin{align}
        \label{eq:ll-cc-mass}
...
\remark{}{
    ...
    %
    For example, the groomed jet mass distribution we consider above is well-behaved and normalized to unity (\(\Sigma(Q^2/2) = 1\)), but the NLO result, obtained when we expand at \(\mathcal{O}(\alpha_s)\), is singular and not normalizable.
    %
    \sam{Fix factors of two!!!}
}
...
%
It is the paradigm of \glslink{piranha}{\textbf{P}ileup and \textbf{I}nfrared \textbf{R}adiation \textbf{A}n\textbf{N}i\textbf{H}il\textbf{A}tion} (\textsc{Piranha}) for jet grooming.
%
...
%
This is one reason why recent work in jet substructure is moving instead towards using energy correlators (see \Chap{ewocs}) to extract the top mass \cite{}.
...
Though we do not investigate grooming methods that utilize these more restrictive analogs of continuity in this work, the invention and understanding of such groomers offer an intriguing and promising avenue for future research.
%
...
%
For each plot, we display the results of the convolution method of \Eq{convolution} using numerical integration and \gls{pqcd} with \(10^6\) phase space samples as well as the parton shower algorithm of \App{partonshower}.
%
...
%
Our parton shower procedure is inspired by \Reff{Larkoski:2013paa} and is exposed in more detail in Problem~\ref{prob:partonshower}.
%
...
% \sam{Turn into a problem, as in jet notes?}
...
%
\footnotetext{
Soft Drop with \(\beta_{\rm SD} \leq 0\) is not collinear safe in grooming mode, used in this work, where jets consisting of a single particle are retained as groomed jets.
%
...
%
\sam{include discussion of hard-cutoff grooming}
%
...
%%
%Finally, we extend our appreciation to an anonymous referee whose thoughtful comments and feedback contributed to improving the quality and clarity of this work and especially of \Sec{discontinuity}.
%%
...
\sam{Use continuity and \gls{ue} mass resolution as a segueway to energy correlations}
