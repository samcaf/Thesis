...
%
The proof of this formula, at least for the simple case of angularities, is the business of \Prob{multiple-emissions}.
...
\makeprob{Recursive Fragmentation Functions}{p2p-fragmentation}{
    ...
    \begin{enumerate}[label=\alph*)]
        ...
        \item
            Show that the recursive fragmentation formula obeys the DGLAP equations of \Eq{dglap-equations-parton-shower} derived through a more complete parton shower approach in \Sec{p2p-fragmentation}.
    \end{enumerate}
}


% TODO: multiple emissions
\makeprob{\(\star\star\) Multiple Emissions \(\star\star\)}{multiple-emissions}{
    By taking a Laplace transform, show that the cumulative distribution for an additive observable \(X(z, \theta)\) with radiator \(R_X\) takes the form
    \begin{align}
        \,,
    \end{align}
    when multiple emissions are included.
...
%    Using the leading-logarithmic principles of the partonic cascade, find the multiplicity distribution of subjets of radius \rsub{} within a jet of radius \Rjet{}.
%    %
%    In other words, find the probability \(\mathbb{P}(n | \rsub)\) that there are \(n\) subjets of radius \rsub{} within a jet of radius \Rjet{}, for all \(n\).

%    \texttt{(Hint:)}
%    %
%    Our solution involves finding a set of coupled differential equations describing how the probabilities \(\mathbb{P}(n | \rsub)\) change as \rsub{} is varied.
%}
