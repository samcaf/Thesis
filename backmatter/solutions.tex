...
\begin{enumerate}
	...
	Generate an angularity, angle, and energy fraction with Steps \ref{item:LL_ps_start}--\ref{item:LL_ztheta_sampling} of the LL parton shower algorithm, with the coupling \(\alpha_s\) of \Eqs{alg_LLang_ratio}{alg_LLang} replaced by a large fixed value for the coupling, \(\hat\alpha = \frac{1}{2}\).
...
\begin{align}
    ...
    \left[
        \int_0^{1} \frac{d\theta'_m}{\theta'_m}
        \int_0^1 dz_m p_i(z_m)
        \frac{\alpha_S(\kappa_m)}{\pi}
...
\begin{align}
    ...
    &= \sum_{n=1}^\infty \prod_{m=1}^n  \left[
        \int_0^{1} \frac{\dd \theta'_m}{\theta'_m}
    ...
    \,.
\end{align}
Now we can take advantage of angular ordering, using Dyson's trick to replace the factors of \(\Theta(\theta_{i-1} - \theta_i)\) by a single factor of \(1 / n!\).
%
Using that the resulting features of the integrand have identical forms for each emission, this yields
\begin{align}
    \widetilde\Sigma(\nu)
    &=
    -\frac{1}{\nu}
...
The stationary value of the integrand is \(\nu = 1 / x + \mathcal{O}(\alpha_s)\), so for small \(x\), \(\nu\) will be very large, and we can approximate
\begin{align}
    1 - e^{\nu X(z,\theta)}
    \approx
    1 - e^{X(z,\theta)/x}
    ...
    \,
\end{align}
as roughly equal to a Heaviside theta function, i.e. that \(\phi(x) := 1 - e^{X(z,\theta)/x} - \Theta(X(z, \theta) - x)\) is small for large values of \(\nu\).
%
We can therefore use the radiator for \(X\) to simplify our results, noting that
\begin{align}
    \int_0^{1} \frac{\dd \theta'}{\theta'}
    \int_0^1 \dd z p_i(z)
    \frac{\alpha_S(\kappa)}{\pi}
...
Furthermore, corrections due to \(\phi(x)\) can also be computed via
\begin{align}
    \int_0^{1} \frac{\dd \theta'}{\theta'}
    \int_0^1 \dd z p_i(z)
    \frac{\alpha_S(\kappa)}{\pi}
...
% ---------------------------------------
\Soln{Multiple Emissions SD}{multiple-emissions}
% ---------------------------------------

\sam{Re-evaluate relevance -- there are things here that aren't elsewhere...}
...
% % ---------------------------------------
% \Soln{Multiplicity of Groomed Subjets}{simple-groomed-multiplicity}
% % ---------------------------------------
